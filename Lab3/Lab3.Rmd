...
A *streak* can be represented by regular expression H*M - i.e. 0 or more hits followed by 1 miss. A streak of length 1 contains exactly 1 hit followed by 1 miss. A streak of length 0 contains 1 miss and 0 hits.
...
There is a clear peak (streak of length 0) on left and a tail (streak of length 4) on the right. This is not a normal distribution, looks like a *unimodal* distribution.
...
In simulation of flipping unfair coin 100 times, 14 flips came up heads.
...
Both distributions should be identicale since we explicitly set the seed of the random number generator. If the seed was removed (or modified) then the streak distribution would be different.

# Exercise 8

> How does Kobe Bryant’s distribution of streak lengths compare to the distribution of streak lengths for the simulated shooter? Using this comparison, do you have evidence that the hot hand model fits Kobe’s shooting patterns? Explain.  

Both distributions look *unimodel*, are right skewed, and have mode equal to 0. Stream lengths of the two distributions, although not identical, are also very similar. For example, unlike the simulated shooter, who has 0 streaks of length 4, Kobe has 1 streak of length 4.
...
We can conclude that Kobe's streaks are random and not due to a hot hand.
