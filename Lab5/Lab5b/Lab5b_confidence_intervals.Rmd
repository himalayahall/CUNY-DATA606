...
In the sample 62%  adults (p_hat) think climate change affects their local community.
...
By 95% confidence we mean there is a 95% probability that the confidence interval contains the true population proportion.
...
In general, the confidence interval will be wider (narrower) for higher (lower) confidence levels. 
...
Using a confidence level of 80%, 43 out of 50 intervals (86%) captured the true population proportion.
...
Increasing the number of bootstrap samples will give a lower bootstrap standard error value, up to a reasonable number. However, the improvement in bootstrap SE with greater than 1000 bootstrap samples is  marginal and not worth the overhead.
