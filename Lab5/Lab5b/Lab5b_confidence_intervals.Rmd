...
n <- 60
samp <- us_adults %>%
  slice_sample(n = n)   # use slice_sample instead of sample_n (superseded)
```
...
In the sample 61.7% adults (p_hat) think climate change affects their local community.
...
Sample proportions of two students would match if and only if they both use the same random number generator (RNG), have the same RNG seed and sample sizes, and called the RNG in same sequence. This is unlikely to occur by chance.  

More likely to occur is that sample proportions of two students will be **similar**. The reason is that the distribution of sample proportions will approximate the Normal distribution (Central Limit Theorem) with 95% within 2 SD of the population mean.
...
By 95% confidence we mean there is a 95% probability that the confidence interval will contain the true population proportion.
...
The confidence interval (0.483, 0.75) does capture the true population proportion (62%) of US adults who think climate change affects their local community.
...
I would expect 95% of intervals generated by other students to capture the true proportion mean. This is because we can expect 95% of the sample intervals to contain the true population proportion.
...
In my plot, 96% (48 out of 50) of intervals include the true population proportion. This proportion is higher than the confidence level (95%). The 95% confidence level is a probability that a sample interval will contain the true population proportion and some variability is to be expected due to random sampling.
    
    ![](https://github.com/himalayahall/CUNY-DATA606/blob/master/Lab5/Lab5b/Lab5b%20Screenshot.png)
...
Let's say we pick 99% confidence level, I would expect this interval to be be wider than the 95% confidence interval. This makes sense because the true population mean is more likely (i.e. greater confidence level) to fall within a wider interval.  

In general, the confidence interval will be wider (narrower) for higher (lower) confidence level. 
...
The confidence interval for the proportion of US Adults who think climate change is affecting their local community with a confidence level of 80% is (0.533, 0.7). In other words, we are 80% confident that the true population proportion will fall within the interval (0.533, 0.7). 
...
Using a confidence level of 80%, the 43 out of 50 intervals (86%) intervals captured the true population proportion.
...
For this exercise let's pick confidence level of 99%. I would expect the confidence interval to be wider than both 95% and 80% confidence levels. It is indeed wider (0.45, 0.783).
    
Using the app with 100 intervals, 98 out of 100 intervals (98%) intervals captured the true population proportion.
...
The confidence interval gets tighter (smaller) as the sample size increases. This is in line with intuition because the the Standard Error contains the sample size in denominator, i.e. greater sample size lowers the SE.
...
Increasing the number of bootstrap samples will give a lower bootstrap standard error value, up to a reasonable number. However, the  improvement in bootstrap standard error beyond 1000 bootstrap samples is marginal at best and not worth the overhead.
