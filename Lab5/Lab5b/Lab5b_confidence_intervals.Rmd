...
## Exercise 7
    ...
    Let's say we pick a 99% confidence level, I would expect this interval to be be wider than the 95% confidence interval. This makes sense because, the true population mean will be more likely (i.e. greater confidence level) to fall within a wider interval.  
...
## Exercise 10
    ...
    For this exercise let's pick a confidence level of 99%. I would expect the confidence interval to be wider than both 95% and 80% confidence levels. It is indeed wider (0.45, 0.783).
    
    Using the app with 100 intervals, the 98 out of 100 intervals (98%) intervals captured the true population proportion.
...
1.  Finally, given a sample size (say, 60), how does the width of the interval change 
    ...
    Increasing the number of boostrap samples will give a better bootstrap standard error value. However, the  improvement in bootstrap standard error beyond 1000 bootstrap samples is marginal and not worth the overhead.
