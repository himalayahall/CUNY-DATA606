...
Sample proportions of any two students would match exactly if they both used the same random number generator (RNG), same RNG seed, and same sample sizes. This is confirmed by the observation that Lab5a instruction Rmd has the exact same proportions as this solution (they both use the same seed).
...
Next we plot the sample distribution. This distribution is Normal, left skewed. Based on this sampling distribution I would guess the true proportion mean to be approximately 0.75 and 0.8.
...
The spread from the sampling distribution for *sample_props15* is greater than the spread for *sample_props150*. One would prefer a sampling with a small spread.
