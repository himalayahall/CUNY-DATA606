...
```{r samp1}
#samp1 <- global_monitor %>%   # superceded by slice_sample (see below)
#  sample_n(50)
...
Let's compute summary stats for the sample distribution. The sample distribution has different proportions from the true population proportion: 74% (Benefits) and 26% (Doesn't Benefit).

```{r}
samp1 %>%
  count(scientist_work = scientist_work) %>%
  mutate(phat = n /sum(n))
```
...
Sample proportions of two students would match if they both used the same random number generator (RNG), had same RNG seed and same sample sizes, and called the RNG in same sequence. This is highly unlikely to occur by chance.

However, if students follow the lab instructions to the letter, including setting the seed to 1234, then it is possible. This is confirmed by the observation that the Lab5a Rmd posted on class website has the exact same proportions as **this** solution (they both use identical RNG and seed).
...
The sample proportion for *samp2* is 84% (Benefits) and 16% (Doesn't Benefit).  

As the sample size increases one expects the sampling variability, i.e. standard error (SE), to decrease. Consequently, sample size of 1000 would have less sampling variability compared to sample size 100 and would provide more accurate estimate of the true population proportion.
...
In *sample_prop50* there are 15000 rows and 4 columns for a total of 60000 elements.  

The sampling distribution looks Normal with a center around 0.20.
...
There are 23 rows in *sample_props_small* and each observation represents that sample's proportion of *Doesn't Benefit*.  

It's surprising there are only 23 rows since we requested 25 samples. My assumption is that 2 samples did not contain any *Doesn't Benefit* observations and were discarded due of divide by 0 error. Increasing the *sample size* will increase the probability of at least 1 *Doesn't Benefit* in every sample.
...
The sample size has a big impact on sample proportions. Increasing (decreasing) the sample size brings the sample proportion closer (further away) to (from) the population proportion.  

Increasing the number of samples has a small impact if sample size is low (e.g. 10) but negligible impact on larger sample sizes.
...
Taking a rather small size of 15 gives a sample proportion of 0.67 of people who think the work scientists do enhances their lives.
...
First we simulate the sampling distribution of proportion of those who think the work scientists do enhances their lives for samples of size 15 by taking 2000 samples from the population of size 15 and computing 2000 sample proportions.
...
Next we plot the sample distribution. This distribution is Normal, slightly left skewed. Based on this sampling distribution I would guess the true proportion mean to be approximately 0.75 and 0.8.
...
Now, lets calculate the true population proportions. The population proportion is 0.8.
...
Next, plot the sample proportion distribution. This sample distribution is also Normal and symmetrical. Based on this sampling distribution I would guess the true population proportion to be 0.8 of those who think the work scientists do enhances their lives.
...
I am assuming that this exercise wants to compare the spread from the sampling distributions for *sample_props15* and *sample_props150*. 

Of the two distributions, *sample_props150* has the smaller spread, shorter left/right tails. Since we are estimating the true population mean we would prefer a sampling distribution that is Normal, symmetrical, and small spread.
