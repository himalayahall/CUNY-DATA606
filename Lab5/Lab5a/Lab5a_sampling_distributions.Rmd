...
Let's compute summary stats for the sample distribution. The sample distribution has different proportions, 74% (Benefits) and 26% (Doesn't Benefit), from the true population proportions 80% (Benefits) and 20% (Doesn't Benefit).
...
In *sample_prop50* there are 15,000 rows (elements), one per sample. The sampling distribution looks Normal with a center around 0.20.
...
It's surprising there are only 23 rows since we requested 25 samples. My guess is that 2 out of 25 samples did not contain even a single *Doesn't Benefit* observation and got discarded due of divide by 0 error. Increasing the *sample size* will increase the probability of at least 1 *Doesn't Benefit* in every sample.
...
The sample size has a big impact on sample proportions. Increasing the sample size makes the sample proportion get closer to the true population proportion.  

Increasing the number of samples has an impact if sample sizes are small (e.g. 10) but only marginal impact with large sample sizes.
...
First we simulate the sampling distribution of proportion of those who think the work scientists do enhances their lives by taking 2000 samples, each of size 15, from the population and computing sample proportions.
...
Of the two distributions, *sample_props150* has the smaller spread, shorter left/right tails. Since we are estimating the true population mean we would prefer a sampling distribution with a small spread.
