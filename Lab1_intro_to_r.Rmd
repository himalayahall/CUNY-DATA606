...
The general trend is a slow increase in the number of girls baptized over the years. A outlier to this trend was between years 1640-1650 when the number of girls baptized fell sharply. 
...
The proportion of boys has been higher than proportion of girls over time.

```{r plot-boy-ratio-vs-year}
...
## Years in Present data set and varible (column) names

```{r present-find-years-in-dataset}
present$year
```

```{r present-find-column-names}
colnames(present)
```

calculate dimensions - number of rows and columns - in the Present dat set once for performance optimization

```{r present-find-dimension-of-dataframe}
p_rows = nrow(present)
p_cols = ncol(present)
```

## Compare dimension of the Arbuthnot and Present data sets. Note: at this point of execution in the R file, a number of computed columns have been adeed to the Arbuthnot data set

```{r compare-arbuthnot-and-present-dimensions}
a_rows = nrow(arbuthnot)
if (a_rows > p_rows) {
    sprintf('arbuthnot has more rows (%d) than present (%d)', a_rows, p_rows)
} else if (a_rows < p_rows) {
    sprintf('arbuthnot has fewer rows (%d) than present (%d)', a_rows, p_rows)
} else {
    print('arbuthnot has same number of rows as present')
}

a_cols = ncol(arbuthnot)
if (a_cols > p_cols) {
    sprintf('arbuthnot has more columns (%d) than present (%d)', a_cols, p_cols)
} else if (a_cols < p_cols) {
    sprintf('arbuthnot has fewer columns (%d) than present (%d)', a_cols, p_cols)
} else {
    print('arbuthnot has same number of columns as present')
}
```

## Calculate ration of boys to girls in Present data set

```{r present-calc-ratio-of-boys-to-girls}
present <- present %>% 
    mutate(boy_to_girl_ratio = boys / girls)
```

## Plot that displays the proportion of boys born over time. 

The Arbuthnot's observation about boys being born in greater proportion than girls holds up in the U.S.
...
## Year of most total births in the USA

compute total births by year

```{r present-compute-total-births}
present <- present %>%
        mutate(total = boys + girls)
```

sort total births in descending order

The year of most total births in the USA was *1961*
