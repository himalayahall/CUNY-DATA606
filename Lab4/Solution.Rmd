...
Again, the plots provide strong evidence that calories from fat for Mcdonals are also close to Normal.
...
What is the probability that an item picked at random at Mcdonalds will have less than 0.5 grams of trans fat?

```{r}
mcmean_sodium <- mean(mcdonalds$trans_fat)
mcmean_sodium

mcsd_sodium   <- sd(mcdonalds$trans_fat)
mcsd_sodium
```

Let's first figur eout if trans fat is a Normal distribution at Macdonalds.
...
First we compute the mean and standard deviation of sodium for both restaurants.
...
Let's plat a histogram to verify right skew. Histogram confirms right skewed. 
